...
class SkillContractTests(unittest.TestCase):
    ...
    def test_default_routes_match_model_policy(self):
        contract = json.loads(
            (ROOT / "tests/expected-contract.json").read_text(encoding="utf-8")
        )
        self.assertEqual(
            contract["default_routes"]["worker"],
            {"model": "gpt-5.6-terra", "thinking": "max"},
        )
        self.assertEqual(
            contract["default_routes"]["critical"],
            {"model": "gpt-5.6-sol", "thinking": "max"},
        )

        policy = (ROOT / "references/routing-policy.md").read_text(encoding="utf-8")
        self.assertIn(
            "| Terra Max | `gpt-5.6-terra` | `max` | 默认 Worker", policy
        )
        self.assertIn(
            "| Sol Max | `gpt-5.6-sol` | `max` | 默认关键任务与升级路由", policy
        )
        self.assertNotIn("| Luna X High |", policy)
        self.assertNotIn("| Sol High |", policy)
