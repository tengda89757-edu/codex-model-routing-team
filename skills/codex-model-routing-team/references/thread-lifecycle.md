...
1. 调用 `create_thread`，显式传入任务包和路由策略选定的模型：默认 Worker 使用 `gpt-5.6-terra / max`，关键任务使用 `gpt-5.6-sol / max`；只有专项任务才使用规定档位的 `gpt-5.6-luna`。同时传入目标环境。
...
如果错误明确指向策略选定的 Terra、Sol 或专项 Luna 模型不可用，或推理强度不兼容，报告本地模型目录与 App 运行时不一致。禁止换成旧模型继续派遣。
